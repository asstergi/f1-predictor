...
path = "path\\to\\SQL\\dump"
os.chdir(path)

# Path to 'wc.exe' on Windows - similar to 'wc' in linux (e.g. "C:\\Program Files (x86)\\GnuWin32\\bin\\wc.exe")
wc_file = "path\\to\\wc.exe"
...
def parse(input_filename, output_filename):
    ...
    if input_filename == "-":
        ...
        num_lines = int(subprocess.check_output([wc_file, "-l", input_filename]).strip().split()[0])
    tables = {}
    current_table = None
    creation_lines = []
    enum_types = []
    ...
    for i, line in enumerate(input_fh):
        ...
        # Inside-create-statement handling
            ...
            if line.startswith('"'):
                ...
                print "_".join((useless, name, definition))
                try:
                ...
                if type=='integer' and extra=='NOT NULL AUTO_INCREMENT':
                    type='SERIAL'
                    extra=''
                elif type=='integer':
                    type = 'int'
                if extra=='NOT NULL DEFAULT \'0000-00-00\'':
                    extra='NOT NULL DEFAULT \'1985-12-24\''
...
if __name__ == "__main__":
    ...
    #input_filename = os.path.join(path, sys.argv[1])
    #output_filename = os.path.join(path, sys.argv[2])
    input_filename = 'initial_sql_name.sql'
    output_filename = 'final_sql_name.sql'
